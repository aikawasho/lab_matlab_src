...
for tp_z =-21:0.1:0
    ...
    for n = 1:length(sp_x)
            ...
            if reflect_on ==1
                P_im = tp_z/wall_z*theory_p_one(k,a,cx,cy,cz,sp_x(n),sp_y(n),im_z(n),wall_z*2);
            end
...
%     end
    ...
    if save_w == 1
        ...
        save(sprintf('./phase/210616/LSGw-25_ob_atref%.1f.mat', tp),'sin_A','phix');
    end
...
%         c.Label.String = 'The Gor’kov potential';
        ...
        hold off
        if save_graph == 1
            saveas(gcf,sprintf('./210521/LSGw-25_2ref30_%.1f.png', tp))
        end
