...
for n = 1:length(sp_x)
        ...
        P_im = 0;
        P0 = theory_p(k,a,cx,cz,cy,sp_x(n),sp_y(n),sp_z(n),0);
        
        if reflect_on ==1
            P_im = theory_p_one(k,a,cx,cz,cy,sp_x(n),sp_y(n),im_z(n),wall_z*2);
        end
...
for n = 1:length(sp_x)
        ...
        P_im = 0;
        P0 = theory_p(k,a,X,Z,Y,sp_x(n),sp_y(n),sp_z(n),0);
        
        if reflect_on ==1
            P_im = theory_p(k,a,X,Z,Y,sp_x(n),sp_y(n),im_z(n),wall_z*2);
        end
