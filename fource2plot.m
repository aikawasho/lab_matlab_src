...
st_Z = zeros(length(z),length(tp_z));
st_Zy = zeros(length(z),length(tp_z));
z_dis = (-20:20);
for num = 1:length(tp_z)
    ...
    %読み込みファイルパス
    file_name = sprintf('./phase/210618/Bow-25%.1f.mat', tp(3));
    %振幅読み込むかどうか
    load_A = 0;
    ...
    P = zeros(size(Y));
        ...
        for n = 1:length(sp_x)
                ...
                if reflect2_on == 1
                    ...
                    P_im2 = theory_p_f3_2d(k,a,0,Y,Z,sp_x2(n),sp_y2(n),sp_z2(n),f_2);
                end
                %if ph_n > 4
                    ISO = phix(ph_n)+pi*xx;
                    P = P+sinA(ph_n)*P00*A*(P0+P_im+P_im2)*exp(1j*ISO);
                %end
